...
$envVariableSets = @(
    @{ SynapseSQLPool = $env:SynapseSQLPool1; DBName = $env:SynapseDW1; LAWorkspaceName = $env:LAWorkspaceName; LAWorkspaceKey =  $env:LAWorkspaceKey},
    @{ SynapseSQLPool =  $env:SynapseSQLPool3; DBName = $env:SynapseDW3; LAWorkspaceName = $env:LAWorkspaceName; LAWorkspaceKey =  $env:LAWorkspaceKey },
    @{ SynapseSQLPool =  $env:SynapseSQLPool2; DBName = $env:SynapseDW2; LAWorkspaceName = $env:LAWorkspaceName; LAWorkspaceKey =  $env:LAWorkspaceKey }
)
...
foreach ($envVariables in $envVariableSets) {
try { 
$SynapseSQLPool=$envVariables.SynapseSQLPool
$DBName=$envVariables.DBName
$LAWorkspaceName=$envVariables.LAWorkspaceName
$LAWorkspaceKey=$envVariables.LAWorkspaceKey
$ConnectionString="Server=tcp:$SynapseSQLPool,1433;Persist Security Info=False;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Initial Catalog=$DBName;"
...
$SqlCmd.CommandText = "select db_name() AS sqlpoolname, execreq.request_id, execreq.session_id, max(nodeexreq.wait_time) AS wait_time , max(nodeexreq.total_elapsed_time) AS total_elapsed_time, `
...
group by nodeexreq.wait_type,  execreq.session_id, execreq.request_id, execreq.command  ;"
...
$SynapsePOC=$dataset | Select-Object sqlpoolname, request_id, session_id,  wait_time, total_elapsed_time,  reads, writes, logical_reads,  wait_type, command   |ConvertTo-Json
